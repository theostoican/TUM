...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        # First layer output - RELu
        first_layer_relu = X @ W1 + b1
        first_layer_out = np.maximum(first_layer_relu, np.zeros_like(first_layer_relu))
        
        # Second layer outputs
        second_layer_out = first_layer_out @ W2 + b2
        ...
        second_layer_out -= np.max(second_layer_out, axis = 1, keepdims = True)
        second_layer_out = np.exp(second_layer_out)
        second_layer_out /= np.sum(second_layer_out, axis = 1, keepdims = True)
        softmax_loss = np.sum(-np.log(second_layer_out[range(N), y])) / N
        loss = softmax_loss + 0.5 * reg * (np.sum(W1 ** 2) + np.sum(W2 ** 2))
        ...
        d_sec_layer = second_layer_out # (N, C)
        d_sec_layer[range(N), y] -= 1
        d_sec_layer = d_sec_layer / N
        
        grads['b2'] = np.sum(d_sec_layer, axis = 0) # (C)
        
        d_first_layer = d_sec_layer @ W2.T * (first_layer_relu >= 0)
        # (N, C) x (C, H) = (N, H)
        
        grads['b1'] = np.sum(d_first_layer, axis = 0)
        # (H)
        
        # W1 - (D, H)
        # W2 - (H, C)
        
        grads['W2'] = first_layer_out.T @ d_sec_layer + reg * W2 
        #(H, N) x (N, C) = (H, C)
        
        grads['W1'] = X.T @ d_first_layer + reg * W1
    ...
    def predict(self, X):
        ...
        first_layer_out = X @ W1 + b1
        first_layer_out[first_layer_out < 0] = 0
        second_layer_out = first_layer_out @ W2 + b2
        y_pred = np.argmax(second_layer_out, axis = 1)
...
def neuralnetwork_hyperparameter_tuning(X_train, y_train, X_val, y_val):
    ...
    for hidden_size in hidden_sizes:
        for lr in lrs:
            ...
            stats = model.train(X_train, y_train, X_val, y_val, num_iters=4000, batch_size=200, learning_rate=lr, learning_rate_decay=0.95, reg=0.3, verbose=True)
            
            train_acc = np.mean(model.predict(X_train) == y_train)
            val_acc = np.mean(model.predict(X_val) == y_val)
            
            print('lr %e hidden_size %e train accuracy: %f val accuracy: %f' % (
              lr, hidden_size, train_acc, val_acc))
