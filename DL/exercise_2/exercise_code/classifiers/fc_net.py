...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3 * 32 * 32, hidden_dim=100, num_classes=10,
        ...
        # Flow of the network: (N, input_dim) x (input_dim, hidden_dim) = 
        # (N, hidden_dim) x (hidden_dim, num_classes) = 
        # (N, num_classes)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # All these layers are RELu layers
        for i in range(1, self.num_layers):
            W = self.params['W%d'%i]
            b = self.params['b%d'%i]
            layers[i], caches[i] = affine_relu_forward(layers[i - 1], W, b)
            X = layers[i]
            
        # The last layer is just affine
        ...
        dout = {}
        # Last layer is affine, compute separately
        Wlast = 'W%d'%(self.num_layers)
        blast = 'b%d'%(self.num_layers)
        dout[self.num_layers - 1], grads[Wlast], grads[blast] = affine_backward(dscores, caches[self.num_layers])
        grads[Wlast] += self.reg * self.params[Wlast]
        
        for i in reversed(range(1, self.num_layers)):
            Wi = 'W%d'%(i)
            bi = 'b%d'%(i)
            dout[i - 1], grads[Wi], grads[bi] = affine_relu_backward(dout[i], caches[i])
